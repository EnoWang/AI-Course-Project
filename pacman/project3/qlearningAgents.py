...
class QLearningAgent(ReinforcementAgent):
  ...
  def update(self, state, action, nextState, reward):
    ...
    diff = reward + dis *nextValue
    mod=(1-alpha)* curValue+alpha * diff
    self.Qvalues[(state,action)]= mod
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def getQValue(self, state, action):
    ...
    for feature in features:
        Q = Q + self.weights[feature]* features[feature]
    return Q
  ...
  def update(self, state, action, nextState, reward):
    ...
    diff = (reward + self.discount * nextValue)- curValue
    for feature in features:
