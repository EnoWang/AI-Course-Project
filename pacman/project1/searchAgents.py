...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            xStart, yStart = state[0]
            dx, dy = Actions.directionToVector(action)
            x, y = int(xStart + dx), int(yStart + dy)
...
def foodHeuristic(state, problem):
    ...
    if len(foodList) > 0:
        ...
        for food in foodList:
            currentCost = mazeDistance(currentPosition, food, gameState)
            # dis=manhattanDistance(currentPosition,food)

            distance = currentCost if (currentCost > distance)  else distance
        result = distance
    return result
