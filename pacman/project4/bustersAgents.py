...
class GreedyBustersAgent(BustersAgent):
  ...
  def findMin(self,items):
        minDis=999999.99
        target=None
        for item in items:
            if items[item]<minDis:
                minDis=items[item]
                target=item
        return target
  ...
  def chooseAction(self, gameState):
    ...
    for position in livingGhostPositionDistributions:
        max=position.argMax()
        ghostsPos[max]=self.distancer.getDistance(pacmanPosition,max)

    target= self.findMin(ghostsPos)
    
    actionDis=util.Counter()
    for action in legal:
        successor=Actions.getSuccessor(pacmanPosition,action)
        actionDis[action]=self.distancer.getDistance(target,successor)

    
    result = self.findMin(actionDis)
    return  result
